...
from tabulardl.models.encoding import (
    LinearNumericalEncoding,
    PeriodicEncoding,
    PiecewiseLinearEncoding,
)
from tabulardl.models.ft_transformer import Transformer
...
def create_initial_representations(
    ...
    assert (x_num is None) == (
        input_num is None
    ), "x_num and input_num should both be None or not None"
    assert (x_cat is None) == (
        input_cat is None
    ), "x_cat and input_cat should both be None or not None"
    ...
    x_temp: List[torch.Tensor] = [
        input_cat(
            torch.zeros(batch_size, 1, device=x_nonnull.device, dtype=torch.int32)
        )
    ]
...
class Model(nn.Module):
    def __init__(
        ...
        d_ffn_factor: float,
        input_num: Optional[
            PeriodicEncoding | PiecewiseLinearEncoding | LinearNumericalEncoding
        ] = None,
        bilinear=True,
    ):
        super().__init__()
        ...
        if self.input_num is None and d_numerical > 0:
            self.input_num = LinearNumericalEncoding(
                in_features=d_numerical, d_token=d_token
            )
        ...
        if len(categories) > 0:
            self.register_buffer(
                "category_offsets",
                torch.tensor([0] + categories[:-1], dtype=torch.int32).cumsum(0),
            )
            self.input_cat = nn.Embedding(
                num_embeddings=sum(categories), embedding_dim=d_token
            )
        ...
        assert (
            self.input_num is not None or self.input_cat is not None
        ), "Expected at least one numerical or categorical input layer"
        ...
        self.transformer = Transformer(
            ...
            d_feedforward=int(d_ffn_factor * d_token),
        ...
        self.output = nn.Sequential(
            nn.LayerNorm(d_output_hidden), nn.ReLU(), nn.Linear(d_output_hidden, d_out)
        )
    ...
    def forward(
        ...
        hidden_states = create_initial_representations(
            x_num,
            x_cat,
            self.input_num,
            self.input_cat,
            self.get_buffer("category_offsets"),
            self.cat_bias,
        )
...
class ModelForMaskedLM(nn.Module):
    def __init__(self, model: Model):
        ...
        self.transform_hidden_state = nn.Sequential(
            nn.Linear(self.d_token, self.d_token), nn.ReLU(), nn.LayerNorm(self.d_token)
        )
        ...
        self.cat_decoders = nn.ModuleList(
            [
                nn.Linear(self.d_token, vocab_size, bias=False)
                for vocab_size in categories
            ]
        )
    ...
    def forward(
        ...
        """
        assert (x_num is None) == (
            self.input_num is None
        ), "x_num and input_num should both be None or not None"
        assert (x_cat is None) == (
            self.input_cat is None
        ), "x_cat and input_cat should both be None or not None"
        assert not (x_num is None and x_cat is None), "expected at least x_num or x_cat"
        ...
        hidden_state = create_initial_representations(
            x_num,
            x_cat,
            self.input_num,
            self.input_cat,
        ...
        cls, hidden_states_num, hidden_states_cat = hidden_state.split(
            [1, d_numerical, d_cat], dim=-2
        )
        hidden_states_num = torch.einsum(
            "bij,ij->bi", hidden_states_num, self.num_weight
        )

        num_loss_fn = nn.MSELoss(reduction="none")
        num_loss_sum = torch.sum(
            num_loss_fn(hidden_states_num, x_num_actual) * x_num_mask
        )
        ...
        cat_loss_sum = torch.stack(
            [
                cat_loss_fn(
                    F.softmax(decoder(hidden_states_cat[i]), dim=-1),
                    x_cat_actual_transpose[i],
                )
