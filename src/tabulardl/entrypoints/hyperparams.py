...
from tabulardl.entrypoints.train import pretrain_masked_lm, set_seed, train_and_test
...
class Objective:
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        d_out = (
            1 if self.split["train"].n_classes <= 2 else self.split["train"].n_classes
        )
        ...
        if model_name == MLP.__name__:
            ...
            d_hidden = (
                [trial.suggest_int("d_layer_hidden", 1, 512)] if n_layers > 2 else []
            )
            ...
            if config["input_num_name"] == PiecewiseLinearEncoding.__name__:
                ...
                input_num = PiecewiseLinearEncoding(
                    boundaries=generate_ple_boundaries(
                        **config["input_num"], train_dataset=self.split["train"]
                    ),
            ...
            _, model = pretrain_masked_lm(
                model,
                config,
                train_dataset=self.train_mlm_dataset,
                val_dataset=self.val_mlm_dataset,
                device=self.device,
            )
        else:
        ...
        result, model = train_and_test(
            model, config, self.split, device=self.device, test=False
        )

        trial.set_user_attr("config", result.config)
        trial.set_user_attr(
            "results",
            {"train": result.train, "validate": result.validate, "test": result.test},
        )

        checkpoint_dir = self.get_checkpoint_dir(trial.study.study_name)
        torch.save(
            model.state_dict(), os.path.join(checkpoint_dir, f"{trial.number}.pt")
        )
