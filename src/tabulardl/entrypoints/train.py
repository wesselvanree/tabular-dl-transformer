...
from tabulardl.utils.data import (
    DatasetSplit,
    TabularDataset,
...
def pretrain_masked_lm(
    model: Model,
    config: Dict,
    train_dataset: TabularMaskedLMDataset,
    val_dataset: TabularMaskedLMDataset,
    device=None,
    trial: Optional[optuna.Trial] = None,
) -> Tuple[float, Model]:
    tqdm.write(f"Performing MaskedLM on {model.__class__.__name__}...")

    patience = config["training"].get("patience_pretrain") or config["training"].get(
        "patience"
    )
    ...
    epochs_progress = tqdm(
        range(config["training"]["n_epochs"]), unit="epoch", leave=False
    )
    ...
    tqdm.write(
        f"Finished Masked LM with best validation loss {best_val_loss} at epoch {best_epoch}"
    )
...
def train_and_test(
    ...
    trial: Optional[optuna.Trial] = None,
    test=True,
) -> Tuple[TrainLoopResult, nn.Module]:
    training_loader = DataLoader(
        split["train"], batch_size=config["training"]["batch_size"]
    )
    validation_loader = DataLoader(
        split["validation"], batch_size=config["training"]["batch_size"]
    )
    ...
    try:
        for epoch in epochs_progress:
            ...
            if (
                ...
                or validation_accuracy > best_metrics.validate["accuracy"]
            ):
                epochs_progress.set_description(
                    f"Best Val. Acc.: {validation_accuracy:.3f}"
                )
            ...
            if (
                ...
                and epoch - best_epoch >= config["training"]["patience"]
            ):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            PiecewiseLinearEncoding.__name__,
    ...
    parser.add_argument(
        "--mask-prob", type=float, default=0.30, help="Mask probability for MaskedLM"
    )
    ...
    for seed in tqdm(range(15)):
        ...
        if model_name == MLP.__name__:
            ...
            if config["input_num_name"] == PiecewiseLinearEncoding.__name__:
                ...
                input_num = PiecewiseLinearEncoding(
                    boundaries=generate_ple_boundaries(
                        **config["input_num"], train_dataset=split["train"]
                    ),
                ...
                raise ValueError(
                    f"Unsupported input_num_name {config['input_num_name']}"
                )

            model = Model(**config["model"], input_num=input_num, bilinear=use_bilinear)
            if do_mlm:
                mask_prob = config["training"].get("mask_prob") or 0.15
                train_mlm_dataset = TabularMaskedLMDataset(
                    split["train"], mask_prob=mask_prob
                )
                val_mlm_dataset = TabularMaskedLMDataset(
                    split["validation"], mask_prob=mask_prob
                )

                _, model = pretrain_masked_lm(
                    model,
                    config,
                    train_dataset=train_mlm_dataset,
                    val_dataset=val_mlm_dataset,
                    device=device_name,
                )
        else:
        ...
        if result.accuracies_per_epoch is not None:
            result.accuracies_per_epoch.to_csv(
                os.path.join(result_dir, "accuracies.csv"),
