...
from sklearn.preprocessing import LabelEncoder, OrdinalEncoder, QuantileTransformer
...
def normalize(
    df: pd.DataFrame,
    normalizer: Optional[QuantileTransformer] = None,
    random_state: Optional[np.random.RandomState | int] = None,
    noise: Optional[float] = 1e-3,
):
    ...
    if noise is not None:
        ...
        df += noise_std * np.random.default_rng(random_state).standard_normal(
            df.values.shape
        )
...
def preprocess_and_split(
    ...
    if df_test is None:
        df_train, df_test = train_test_split(
            df_train, test_size=test_size, random_state=random_state - 1
        )

    dataset = TabularDataset(
        df_train, y_col=y_col, random_state=random_state, device=device
    )
    ...
    validation_dataset = TabularDataset(
        ...
        encoder_y=encoder_y,
        encoder_x_cat=encoder_x_cat,
        x_num_normalizer=x_num_normalize,
        device=device,
    )
...
class TabularDataset(Dataset):
    def __init__(
        ...
        self.x_num, self.x_num_normalizer = normalize(
            self.x_num, normalizer=x_num_normalizer, random_state=random_state
        )
            ...
            if append_none_class_to_all:
                for i, categories in enumerate(self.encoder_x_cat.categories_):
                    if categories[-1] is not None:
                        self.encoder_x_cat.categories_[i] = np.append(
                            categories, [None]
                        )

        self.x_cat = pd.DataFrame(
            self.encoder_x_cat.transform(self.x_cat),
            columns=self.x_cat.columns,
            dtype="int64",
        )
    ...
    @staticmethod
    def from_adult(dir: str | Path, random_state: Optional[int] = None, device=None):
        names = [
            "age",
            "work_class",
            "fnlwgt",
...
class TabularMaskedLMDataset(Dataset):
    def __init__(self, dataset: TabularDataset, mask_prob=0.15):
        ...
        self.y, self.x_num, self.x_cat = next(
            iter(DataLoader(dataset, batch_size=len(dataset)))
        )
    ...
    def __getitem__(self, item: int):
        return (
            self.x_num[item],
            self.x_cat[item],
            self.x_num_mask[item],
            self.x_cat_mask[item],
        )
...
def generate_ple_boundaries(
    train_dataset: TabularDataset | Subset, n_bins=14
) -> torch.Tensor:
    """
    ...
    for column in x_num_transpose:
        ...
        feature_boundaries = [
            torch.as_tensor(tree.threshold[i], dtype=torch.float32, device=device)
